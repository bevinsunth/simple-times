...
interface PDFReportProps {
  timesheetData: TimeEntryData[];
}

const styles = StyleSheet.create({
  page: { backgroundColor: '#E5E4E2', fontSize: 11, paddingTop: 0, paddingLeft: 40, paddingRight: 50, lineHeight: 1.5, flexDirection: 'column' },
  spaceBetween: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', color: "#3E3E3E" },
  titleContainer: { flexDirection: 'row', marginTop: 24 },
  reportTitle: { fontSize: 10, textAlign: 'center' },
  ...
  tbody: { fontSize: 7, paddingTop: 4, paddingLeft: 7, flex: 1, borderColor: 'whitesmoke', borderRightWidth: 1, borderBottomWidth: 1 },
});
        ...
        <View style={styles.table}>
          ...
          {timesheetData.map((entry, index) => (
            <View style={styles.tableRow} key={index}>
              ...
              <View style={styles.tbody}>
                <Text style={styles.tableCell}>{entry.hours == 0 ? "" : entry.hours}</Text>
              </View>
            </View>
          ))}
        </View>
