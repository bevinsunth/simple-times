...
interface CollectionOperations<T extends Models.Document> {
  ...
  create: (documentId: string, data: Omit<T, keyof Models.Document>) => Promise<T>;
  get: (documentId: string) => Promise<T>;
  update: (documentId: string, data: Omit<T, keyof Models.Document>) => Promise<T>;
}
...
function createCollectionOperations<T extends Models.Document>(
  ...
  return {
    ...
    create: async (documentId: string, data: Omit<T, keyof Models.Document>): Promise<T> => {
      const { databases } = createDatabasesClient();
    ...
    update: async (documentId: string, data: Omit<T, keyof Models.Document>): Promise<T> => {
      const { databases } = createDatabasesClient();
...
export async function GetDbOperations<T extends Models.Document>(
  ...
  return createCollectionOperations<T>(collection.databaseId, collection.collectionId);
}
